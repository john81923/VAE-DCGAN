...
class ConvVAE():
  def __init__(self, batch_size=64,
    ...
    self.x_raw = tf.placeholder(tf.float32, [batch_size, self.y_dim, self.x_dim, 3])
  ...
  def _recognition_network(self, image):
    ...
    z_mean = linear(tf.reshape(h3, [self.batch_size, -1]), self.z_dim, 'd_h3_lin_mean')
    z_log_sigma_sq = linear(tf.reshape(h3, [self.batch_size, -1]), self.z_dim, 'd_h3_lin_log_sigma_sq')
  ...
  def _generator_network(self, z):
    ...
    # The transformation is parametrized and can be learned.
    s2, s4, s8, s16 = int(self.x_dim//2), int(self.x_dim/4), int(self.x_dim//8), int(self.x_dim//16) # 32,16,8,4
    # project `z` and reshape
    
    self.z_, self.h0_w, self.h0_b = linear(z, self.gf_dim*4*8*8, 'g_h0_lin', with_w=True)
    print self.z_.shape
    self.h0 = tf.reshape(self.z_, [-1, 8, 8, self.gf_dim * 4])
    h0 = tf.nn.relu(self.g_bn0(self.h0))

    self.h1, self.h1_w, self.h1_b = deconv2d(h0,
        [self.batch_size, s4, s4, self.gf_dim*2], name='g_h1', with_w=True)
    h1 = tf.nn.relu(self.g_bn1(self.h1))
    print self.h1.shape

    h2, self.h2_w, self.h2_b = deconv2d(h1,
        [self.batch_size, s2, s2, self.gf_dim*1], name='g_h2', with_w=True)
    h2 = tf.nn.relu(self.g_bn2(h2))

    h3, self.h3_w, self.h3_b = deconv2d(h2,
        [self.batch_size, self.y_dim, self.x_dim, 3], name='g_h3', with_w=True)
  ...
  def _create_loss_optimizer(self):
    ...
    self.mse = tf.losses.mean_squared_error(labels=orig_image,predictions=new_image )
    ...
    self.cost = self.mse + 1e-6*self.vae_loss_kl   # average over batch
    ...
    # Use ADAM optimizer
    self.optimizer = tf.train.AdamOptimizer(self.learning_rate, beta1=0.5 ).minimize(self.cost, var_list=self.t_vars)
  ...
  def partial_fit(self, X):
    ...
    opt, cost, mse, vae_loss_kl,new_image ,z_log_sigma_sq= self.sess.run((self.optimizer, self.cost, self.mse, self.vae_loss_kl, 
      self.new_image,self.z_log_sigma_sq), feed_dict={self.x_raw: X})
    return cost, mse, vae_loss_kl , new_image,z_log_sigma_sq
  def testing_fit(self, X):
    """Train model based on mini-batch of input data.

    Return cost of mini-batch.
    """

    new_image ,z= self.sess.run((self.new_image,self.z ), feed_dict={self.x_raw: X})
    return new_image ,z
  def mse_fit(self, X):
    """Train model based on mini-batch of input data.
    ...
    new_image ,mse,z= self.sess.run((self.new_image,self.mse,self.z ), feed_dict={self.x_raw: X})
    return new_image,mse ,z
  def testing_1_4(self, X):
    """Train model based on mini-batch of input data.

    Return cost of mini-batch.
    """

    new_image = self.sess.run((self.new_image ), feed_dict={self.z: X})
    return new_image 
